...
class Bridge(Infra):
    ...
    def __init__(self, unique_id, model, length=0,
        ...
        """ 
        The delay time is calculated based on the length of the bridge and the condition of the bridge.
         Over 200 m Triangular(1, 2, 4) hours
         Between 50 and 200 m Uniform(45, 90) minutes
         Between 10 and 50 m Uniform(15, 60) minutes
         Under 10 m Uniform(10, 20) minutes
           """
        np.random.seed(int(self.random.random()*100))
        if self.length > 200:
            self.delay_time = np.random.triangular(1, 2, 4)*60
        elif self.length > 50:
            self.delay_time = np.random.uniform(45, 90)
        elif self.length > 10:
            self.delay_time = np.random.uniform(15, 60)
        else:
            self.delay_time = np.random.uniform(10, 20)
                        

    # TODO
    def get_delay_time(self,vehicle):
        """
        Returns the delay time (in ticks) caused by this bridge
        Parameters:
        -----------
        None
        Returns:
        --------
        int
            the delay time (in ticks) caused by this bridge
        """
        delay_time = 0
        # set seed for numpy
        np.random.seed(int(self.random.random()*100))
        #conditions can be A, B, C, D; set delay time if random number is less than the probability        
        if self.condition == 'A':            
            delay_time = self.delay_time if np.random.uniform(0,100) < self.model.cat_a_prob else 0            
        elif self.condition == 'B':
            delay_time = self.delay_time if np.random.uniform(0,100) < self.model.cat_b_prob else 0
        elif self.condition == 'C':
            delay_time = self.delay_time if np.random.uniform(0,100) < self.model.cat_c_prob else 0
        elif self.condition == 'D':
            delay_time = self.delay_time if np.random.uniform(0,100) < self.model.cat_d_prob else 0

        # add delay time to total delay time of bridge
        self.total_delay_time += delay_time

        return delay_time             
        
    def get_total_delay_time_at_bridge(self):
        """
        Returns the total delay time (in ticks) caused by this bridge
        Parameters:
        -----------
        None
        Returns:
        --------
        int
            the total delay time (in ticks) caused by this bridge
        """
...
class Sink(Infra):
    ...
    driving_times: list
        list of driving times of vehicles
    ...
    ...
    def __init__(self, unique_id, model, length=0,
                 name='Unknown', road_name='Unknown', condition='Unknown'):
        # call init function of Infra class
        super().__init__(unique_id, model, length, name, road_name)
        # init list to store driving times of vehicles
        self.driving_times = []

    def remove(self, vehicle):
        """
        Removes a vehicle from the model
        Parameters:
        -----------
        vehicle: Vehicle
        ...
        None
        """
        # add time to driving times of sink
        self.driving_times.append(vehicle.driving_time())
        # remove vehicle from model
...
class Source(Infra):
    ...
    def step(self):
        """
        Generates a truck if the generation frequency is reached
        """
...
class Vehicle(Agent):
    ...
    def drive(self):
        """
        Vehicle drives to the next object
        """
    ...
    def drive_to_next(self, distance):
        ...
        if isinstance(next_infra, Sink):
            ...
            self.waiting_time = next_infra.get_delay_time(self)
            if self.waiting_time > 0:
    ...
    def driving_time(self):
        """
        Return the driving time of the vehicle
        Parameters:
            None
        Returns:
            int: the driving time in minutes
        """
