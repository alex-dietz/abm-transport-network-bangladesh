...
class BangladeshModel(Model):
    ...
    def __init__(self, seed=None, x_max=500, y_max=500, x_min=0, y_min=0,scenario=[0,0,0,0]):
        ...
        self.sources = []
        self.sinks = []
        self.cat_a_prob = scenario[0]
        self.cat_b_prob = scenario[1]
        self.cat_c_prob = scenario[2]
        self.cat_d_prob = scenario[3]        
        self.generate_model()
    ...
    def generate_model(self):
        ...
        df = pd.read_csv('input/infrastructure.csv')
...
def average_driving_time(model):
        """
        Calculate the average driving time of all vehicles
        """
        driving_times = []
        for agent in model.schedule.agents:
            if isinstance(agent, Vehicle):                                
                if agent.removed_at_step is not None:                    
                    driving_times.append(agent.driving_time())        
        return np.mean(driving_times)
# EOF -----------------------------------------------------------
